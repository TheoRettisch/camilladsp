...
fn legacy_signed_sum_matches_bitwise_across_chunks() {
    for mode in [None, Some(config::MonitorMode::Sum)] {
        let mut parameters: config::CompressorParameters = yaml_serde::from_str(
            "channels: 3
...
",
        ...
        let mut actual_processor = Compressor::from_config("legacy", parameters, 48000, 16);
        ...
        for block in 0..8 {
            ...
            for &ch in &legacy.process_channels {
                ...
                legacy.apply_clipper(&mut expected.waveforms[ch]);
            }
            for (a, b) in actual
                .waveforms
                .iter()
...
fn all_mode_updates_preserve_smoothed_level() {
    ...
    for before in modes {
        for after in modes {
            ...
            let mut p = Compressor::from_config("p", parameters.clone(), 48000, 8);
            ...
            p.update_parameters(config::Processor::Compressor {
                description: None,
                parameters,
            });
...
fn zero_channel_count_is_rejected() {
    let parameters: config::CompressorParameters = yaml_serde::from_str(
        "channels: 0\nattack: 3\nattack_unit: samples\nrelease: 19\nrelease_unit: samples\nthreshold: -20\nfactor: 2\n",
    ).unwrap();
    assert!(validate_compressor(&parameters).is_err());
}
